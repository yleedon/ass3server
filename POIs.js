...
module.exports.get_POIs_By_Category = mget_POIs_By_Category;
...
function get_POI_info(POI_ID) {
    return DButilsAzure.execQuery(`SELECT * FROM POIs WHERE ID='${POI_ID}'`)
        ...
        ).catch(err => {return new Promise((resolve, reject) => reject({'code':400, 'msg':err}))});
}
...
function mget_POIs_By_Category(Category){
    console.log("m get pois by category");
...
async function get_POIs(min_rating) {
    ...
    return DButilsAzure.execQuery(`SELECT * FROM POIs WHERE Rank >= ${min_rating}`)
        ...
        .catch(err => {
            return new Promise((resolve, reject) => resolve({'code': 400, 'msg': 'get_POIs:\n' + err}))
        });
}
...
async function get_favorites(UserName) {
    ...
    return DButilsAzure.execQuery(`SELECT * FROM POIs WHERE ID IN (SELECT POI_ID FROM FavoritePOIs WHERE UserName='${UserName}') `)
        ...
        .catch(err => {
            return new Promise((resolve, reject) => resolve({'code': 400, 'msg': 'get_favorites:\n' + err}))
        });
}